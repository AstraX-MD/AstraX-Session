...
// CONTEXT INFO - OLD STYLE: FORWARDED + VIEW CHANNEL BUTTON
...
// SAFE SEND WITH OLD STYLE CONTEXT - FOR WELCOME + INSTRUCTIONS
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open' && !isSessionGenerated) {
      ...
      try {
        ...
        if (!hasAutoFollowed && CHANNEL_JID) {
        ...
        // 3. SEND WELCOME - WITH OLD STYLE CONTEXT: FORWARDED + VIEW CHANNEL
        const welcomeText = `> ╭─────〔 ${BOT_NAME} CORE 〕─────┈⊷
> │ 𐂂 Engine  : Active
> │ 𐂂 AI Core : Online
> │ 𐂂 Status  : Running
> │ 𐂂 Plugins : 500+
...
> ${BOT_NAME} IS HERE
> 
> Connection successful! You have been subscribed to ${CHANNEL_NAME} for updates.
> 
> Preparing your Session ID...`

        await sendWithOldContext(sock, sock.user.id, welcomeText, userId)
        console.log(`[${userId}] Welcome message sent with new style`)
        ...
        // 6. SEND INSTRUCTIONS - WITH NEW STYLE
        const instructionsText = `> ╭────〔 SELECT CATEGORY 〕────┈⊷
> │➪ 1. DEPLOY
> │➪ 2. SETUP
> │➪ 3. FORK REPO
> │➪ 4. ENV SESSION_ID
...
> github.com/xbotmanager-cell/astrax
> 
...
> Name: \`SESSION_ID\`
> Value: _Paste the code from above message_
> 
> *📢 Need Help?*
> Tap "View channel" below for updates`

        await sendWithOldContext(sock, sock.user.id, instructionsText, userId)
        console.log(`[${userId}] Instructions sent with new style`)

        socket.emit('success', finalSessionId)
        socket.emit('status', 'Session and instructions sent to WhatsApp')
    ...
    if (connection === 'close') {
      ...
      // CRITICAL: ALLOW MULTI-DEVICE - If session used elsewhere, disconnect this one
      ...
      // REPO 2 LOGIC: Allow connection to be taken over
...
server.listen(PORT, () => {
  console.log(`==> AstraX Server operational on port ${PORT}`)
